...
def extract_sql_identifiers(dataset):
    """Extrait les noms de tables et colonnes SQL du dataset et normalise les mots-clés SQL."""
    identifiers = set()
    sql_pattern = re.compile(r"\b[A-Za-z_][A-Za-z0-9_]*\b")  # Détecte les mots SQL
    ...
    for data in dataset:
        if 'output' in data:  # Vérifie que la clé 'output' est présente
            ...
            # Filtrer pour éviter d'ajouter les mots-clés SQL comme tokens
            for word in words:
...
def tokenize_dataset(dataset, tokenizer):
    """Tokenise les données et affiche les détails."""
    tokenized_dataset = []
    ...
    for i, data in enumerate(dataset):
        ...
        # 🔍 Affichage des détails après tokenisation
        print("\n🔍 Exemple de donnée tokenisée :")
        print(f"📌 Texte d'origine : {input_text}")
        print(f"🟢 Tokens : {tokenizer.convert_ids_to_tokens(tokenized['input_ids'][0])}")
        print(f"🔢 IDs : {tokenized['input_ids']}")
        print(f"🎭 Masque d'attention : {tokenized['attention_mask']}")
